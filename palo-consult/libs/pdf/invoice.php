...
/*
function GetData ($nombreId,$hours,$comentario1,$comentario2,$comentario3){
...
function addEcheance( $date )
{
    ...
    $this->SetXY( $r1 + ($r2 - $r1)/2 - 5 , $y1+1 );
    $this->SetFont( "Arial", "B", 10);
    $this->Cell(10,4, "CANDIDATE", 0, 0, "C");
    $this->SetXY( $r1 + ($r2-$r1)/2 - 5 , $y1 + 5 );
    $this->SetFont( "Arial", "", 10);
    $this->Cell(10,5,$date, 0,0, "C");
}
...
function addReference($coach_nombre)
{
    $this->SetFont( "Arial", "", 10);
    $length = $this->GetStringWidth( "The following report is made by our Coach " . $coach_nombre);
    $r1  = 10;
    $r2  = $r1 + $length;
    $y1  = 90;
    $y2  = $y1+5;
    $this->SetXY( $r1 , $y1 );
    $this->Cell($length,4, "The following report is made by our Coach " . $coach_nombre);
}


function addCols( $tab, $comentario_1 )
{
    global $colonnes;
    ...
    $this->SetXY(10,106);
    $this->Multicell(190,7,$comentario_1,0,'L',0);
    $colX = $r1;
    $colonnes = $tab;
...
function addCols2( $tab, $comentario_2 )
{
    global $colonnes;
    ...
    $this->Line( $r1, $y1+6, $r1+$r2, $y1+6);
    $this->SetXY(10,156);
    $this->Multicell(190,7,$comentario_2,0,'L',0);
    $colX = $r1;
    $colonnes = $tab;
    while ( list( $lib, $pos ) = each ($tab) )
    {
...
function addCols3( $tab, $comentario_3 )
{
    global $colonnes;
    ...
    $this->Line( $r1, $y1+6, $r1+$r2, $y1+6);
    $this->SetXY(10,206);
    $this->Multicell(190,7,$comentario_3,0,'L',0);
    $colX = $r1;
    $colonnes = $tab;
    while ( list( $lib, $pos ) = each ($tab) )
    {
